...
//        source.subscribe(new PublisherFlatMapMain3<>(s, mapper, delayError, maxConcurrency, mainQueueSupplier, prefetch, innerQueueSupplier));
    ...
    extends SpscFreeListTracker<PublisherFlatMapInner2<R>>
    implements Subscriber<T>, Subscription, Receiver, MultiReceiver, Requestable, Completable, Producer,
               Cancellable, Backpressurable, Introspectable {
        ...
        @SuppressWarnings("unchecked")
        @Override
        protected PublisherFlatMapInner2<R>[] empty() {
            return EMPTY;
        }
        
        @SuppressWarnings("unchecked")
        @Override
        protected PublisherFlatMapInner2<R>[] terminated() {
            return TERMINATED;
        }
        ...
        protected PublisherFlatMapInner2<R>[] newArray(int size) {
            return new PublisherFlatMapInner2[size];
        }
        
        @Override
        protected void setIndex(PublisherFlatMapInner2<R> entry, int index) {
            entry.index = index;
        }
        
        @Override
        protected void unsubscribeEntry(PublisherFlatMapInner2<R> entry) {
            entry.cancel();
        }
        ...
        public void onNext(T t) {
            ...
            if (p instanceof Supplier) {
                ...
                PublisherFlatMapInner2<R> inner = new PublisherFlatMapInner2<>(this, prefetch);
                if (add(inner)) {

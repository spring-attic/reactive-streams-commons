...
/**
 * A singleton Subscription that represents a cancelled subscription instance
 * and should not be leaked to clients as it represents a terminal state.
 * <br>
 * If algorithms need to hand out a subscription, replace this with
 * {@link EmptySubscription#INSTANCE} because there is no standard
 * way to tell if a Subscription is cancelled or not otherwise.
 */
public enum CancelledSubscription implements Subscription {
    ...
    /**
     * Atomically sets a new Subscription in the field and cancels any previous
     * Subscription or cancels the new Subscription if the field holds the cancelled instance.
     * @param field
     * @param next
    ...
    /**
     * Atomically sets the first Subscription in field or returns false if the
     * field was not null and not holding the cancelled instance.
     * @param field
     * @param first
     * @return
     */
    ...
    /**
     * Atomically replaces the Subscription in field with the new Subscription but
     * does not cancel the original Subscription.
     * @param field
     * @param next
     * @return false if the field contains the cancelled Subscription
     */
    ...
    /**
     * Atomically replaces the Subscription in field with the cancelled instance
     * and cancels the original Subscription.
     * @param field
     ...
     * @return
     */
    public static <T> boolean setOnce(AtomicReferenceFieldUpdater<T, Subscription> updater, T instance, Subscription first) {
        Subscription current = updater.get(instance);
        if (current == INSTANCE) {
            if (first != null) {
                first.cancel();
     ...
     * @return false if the field contains the cancelled Subscription
     */
    public static <T> boolean replace(AtomicReferenceFieldUpdater<T, Subscription> updater, T instance, Subscription next) {
        for (;;) {
            Subscription current = updater.get(instance);
            if (current == INSTANCE) {
                if (next != null) {

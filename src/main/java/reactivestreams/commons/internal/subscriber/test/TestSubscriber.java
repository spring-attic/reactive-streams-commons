...
public class TestSubscriber<T> extends SubscriberDeferSubscription<T, T> {
	...
	public final TestSubscriber<T> assertValue(T value) {
		...
		if (s > 1) {
			assertionError("More values received: expected = " + valueAndClass(value) + ", actual = " + "[" + s + "] " + values,
					null);
		}
		return this;
	}
	...
	public final TestSubscriber<T> assertValues(T... values) {
		...
		for (int i = 0; i < s; i++) {
			...
			if (!Objects.equals(t1, t2)) {
				assertionError("Values at " + i + " differ: expected = " + valueAndClass(t1) + ", actual = " + valueAndClass(
						t2), null);
			}
		}
	...
	public final TestSubscriber<T> assertValueSequence(Iterable<? extends T> expectedSequence) {
		...
		for (; ; ) {
			...
			if (n1 && n2) {
				...
				if (!Objects.equals(t1, t2)) {
					assertionError("The " + i + " th elements differ: expected = " + valueAndClass(t2) + ", actual =" + valueAndClass(
							t1), null);
				}
				i++;
			}
	...
	public final TestSubscriber<T> assertError(Throwable e) {
		...
		if (s == 1) {
			if (!Objects.equals(e, errors.get(0))) {
				assertionError("Errors differ: expected = " + valueAndClass(e) + ", actual = " + valueAndClass(errors.get(
						0)), null);
			}
		}
		if (s > 1) {

 ...
 */
package reactivestreams.commons.internal.subscriber.test;
 ...
 * <p>
 * You can call {@link #request(long)} and {@link #cancel()} from any thread or from within the overridable
 * methods but you should avoid calling the assertXXX methods asynchronously.
 * 
    ...
    public final TestSubscriber<T> assertValue(T value) {
        ...
        if (s == 1) {
            if (!Objects.equals(value, values.get(0))) {
                assertionError("Values differ: expected = " + valueAndClass(value) + ", actual = " + valueAndClass(values.get(0)), null);
            }
        }
        if (s > 1) {
    ...
    public final TestSubscriber<T> assertValues(T... values) {
        ...
        for (int i = 0; i < s; i++) {
            ...
            if (!Objects.equals(t1, t2)) {
                assertionError("Values at " + i + " differ: expected = " + valueAndClass(t1) + ", actual = " + valueAndClass(t2), null);
            }
        }
    ...
    public final TestSubscriber<T> assertError(Throwable e) {
        ...
        if (s == 1) {
            if (!Objects.equals(e, errors.get(0))) {
                assertionError("Errors differ: expected = " + valueAndClass(e) + ", actual = " + valueAndClass(errors.get(0)), null);
            }
        }
        if (s > 1) {
            assertionError("Multiple errors: " + s, null);
    ...
    public final TestSubscriber<T> assertError(Class<? extends Throwable> clazz) {
        ...
        if (s == 1) {
            ...
            if (!clazz.isInstance(e)) {
                assertionError("Error class incompatible: expected = " + clazz.getSimpleName()
                + ", actual = " + valueAndClass(e), null);
            }
        }
        if (s > 1) {
            assertionError("Multiple errors: " + s, null);
    ...
    public final TestSubscriber<T> assertErrorCause(Throwable e) {
        ...
        if (s == 1) {
            ...
            if (!Objects.equals(e, cause)) {
                assertionError("Errors differ: expected = " + valueAndClass(e) + ", actual = " + valueAndClass(cause), null);
            }
        }
        if (s > 1) {
            assertionError("Multiple errors: " + s, null);
    ...
    public final TestSubscriber<T> assertErrorCause(Class<? extends Throwable> clazz) {
        ...
        if (s == 1) {
            ...
            if (clazz.isInstance(cause)) {
                assertionError("Error class incompatible: expected = " + clazz.getSimpleName() + ", actual = " + valueAndClass(cause), null);
            }
        }
        if (s > 1) {
            assertionError("Multiple errors: " + s, null);
